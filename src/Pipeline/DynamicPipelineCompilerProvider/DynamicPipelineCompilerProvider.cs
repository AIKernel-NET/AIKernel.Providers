...
/// <summary>
/// [EN] Official AIKernel external provider for dynamic semantic pipeline compilation.
/// [JA] dynamic semantic pipeline compilation 向けの AIKernel 公式外部 Provider です。
/// </summary>
public sealed class DynamicPipelineCompilerProvider(
    DynamicPipelineCompilerSettings settings) : IProvider
{
    private static readonly DynamicPipelineCompilerProviderCapabilities Capabilities = new();
    private readonly DynamicPipelineCompilerSettings _settings = settings ?? throw new ArgumentNullException(nameof(settings));
    ...
    /// </summary>
    public DynamicPipelineCompilerProvider()
        : this(new DynamicPipelineCompilerSettings())
    {
    }
    ...
    public CapabilityModuleDescriptor ToCapabilityDescriptor()
        => DynamicPipelineCompilerCapabilityContracts.ToContract(
            new DynamicPipelineCompilerCapabilityDescriptor(
                ProviderId,
                _settings.DslSchemaVersion,
                _settings.ToMetadata()));

}

internal sealed class DynamicPipelineCompilerProviderCapabilities : IProviderCapabilities
{
