...
/// <summary>
/// [EN] Official AIKernel external provider for deterministic chat history records.
/// [JA] deterministic chat history record 向けの AIKernel 公式外部 Provider です。
/// </summary>
public sealed class ChatHistoryProvider(
    ChatHistorySettings settings,
    IReadOnlyList<ChatHistoryRecord>? records = null) : IProvider
{
    private static readonly ChatHistoryProviderCapabilities Capabilities = new();
    private readonly ChatHistorySettings _settings = settings ?? throw new ArgumentNullException(nameof(settings));
    private readonly IReadOnlyList<ChatHistoryRecord> _records = records ?? [];
    ...
    public ChatHistoryProvider()
        : this(new ChatHistorySettings())
    {
    }

    /// <summary>
    /// [EN] Initializes the provider from a JSON record file.
    /// [JA] JSON record file から Provider を初期化します。
    /// </summary>
    public ChatHistoryProvider(string jsonPath)
        : this(new ChatHistorySettings(), LoadRecords(jsonPath))
    {
    }
    ...
    /// <summary>
    /// [EN] Returns all records in deterministic source order.
    /// [JA] すべての record を決定論的 source order で返します。
    /// </summary>
    public IReadOnlyList<ChatHistoryRecord> GetAll() => _records;

    /// <summary>
    /// [EN] Returns records matching a role.
    /// [JA] role に一致する record を返します。
    /// </summary>
    ...
    /// <summary>
    /// [EN] Returns records in deterministic source order.
    /// [JA] record を決定論的 source order で返します。
    /// </summary>
    public IEnumerable<ChatHistoryRecord> GetRecords()
        => _records;

    /// <summary>
    /// [EN] Returns the latest record from a sequence.
    /// [JA] sequence から最新 record を返します。
    /// </summary>
    public static ChatHistoryRecord? GetLatest(IEnumerable<ChatHistoryRecord> records)
    ...
    public CapabilityModuleDescriptor ToCapabilityDescriptor()
        => ChatHistoryCapabilityContracts.ToContract(
            new ChatHistoryCapabilityDescriptor(
                ProviderId,
                _settings.ToMetadata()));

    private static IReadOnlyList<ChatHistoryRecord> LoadRecords(string jsonPath)
    {
        var json = File.ReadAllText(jsonPath);
        return JsonSerializer.Deserialize<List<ChatHistoryRecord>>(json)
               ?? throw new InvalidOperationException("Invalid chat history JSON.");
    }

}

internal sealed class ChatHistoryProviderCapabilities : IProviderCapabilities
{
