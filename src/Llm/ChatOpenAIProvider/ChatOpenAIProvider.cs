...
/// <summary>
/// [EN] Official AIKernel external provider for OpenAI-compatible endpoints.
/// [JA] OpenAI 互換 endpoint 向けの AIKernel 公式外部 Provider です。
/// </summary>
public sealed class ChatOpenAIProvider(
    ChatOpenAISettings settings,
    ChatOpenAIClient? client = null) : IModelProvider
{
    private static readonly ChatOpenAIProviderCapabilities Capabilities = new();
    private readonly ChatOpenAISettings _settings = settings ?? throw new ArgumentNullException(nameof(settings));
    private readonly ChatOpenAIClient? _client = client;
    ...
    /// <summary>
    /// [EN] Provides the GenerateAsync provider contract member.
    /// [JA] GenerateAsync の provider contract member を提供します。
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        if (!_initialized)
        {
            throw new InvalidOperationException("Provider must be initialized before generation.");
        }

        if (_client is null)
        {
            throw new InvalidOperationException("ChatOpenAIClient is required for generation.");
        }

        return _client.CompleteAsync(messages, cancellationToken);
    }

    /// <summary>
    /// [EN] Provides the StreamGenerateAsync provider contract member.
    /// [JA] StreamGenerateAsync の provider contract member を提供します。
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var text = await GenerateAsync(messages, cancellationToken).ConfigureAwait(false);
        await onChunk(text).ConfigureAwait(false);
    }

    /// <summary>
    /// [EN] Provides the AnswerAsync provider contract member.
    /// [JA] AnswerAsync の provider contract member を提供します。
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            messages.Add(new ModelMessage("system", context));
        }

        messages.Add(new ModelMessage("user", question));
    ...
    public CapabilityModuleDescriptor ToCapabilityDescriptor()
        => ChatOpenAICapabilityContracts.ToContract(
            new ChatOpenAICapabilityDescriptor(
                ProviderId,
                _settings.Model,
                _settings.ToMetadata()));

    private sealed record ModelMessage(
        string Role,
        string Content) : IModelMessage;
}

internal sealed class ChatOpenAIProviderCapabilities : IProviderCapabilities
{
