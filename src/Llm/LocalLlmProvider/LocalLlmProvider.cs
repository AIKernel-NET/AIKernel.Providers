...
/// <summary>
/// [EN] Official AIKernel external provider for local LLM runtimes.
/// [JA] local LLM runtime 向けの AIKernel 公式外部 Provider です。
/// </summary>
public sealed class LocalLlmProvider(
    LocalLlmSettings settings) : IProvider
{
    private static readonly LocalLlmProviderCapabilities Capabilities = new();
    private readonly LocalLlmSettings _settings = settings ?? throw new ArgumentNullException(nameof(settings));
    ...
    public CapabilityModuleDescriptor ToCapabilityDescriptor()
        => LocalLlmCapabilityContracts.ToContract(
...
internal sealed class LocalLlmProviderCapabilities : IProviderCapabilities
{
